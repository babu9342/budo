import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { useSelector, useDispatch } from 'react-redux';
import { socketService } from '../services/socket';
import { api } from '../services/api';
import { updateLobby, setRoom } from '../store/roomSlice';
import { setGameState } from '../store/gameSlice';
import Navbar from '../components/Navbar';
import { 
  Users, Share2, Play, Crown, CheckCircle2, 
  Clock, Bot, Copy, Check, MessageSquare
} from 'lucide-react';
import { sound } from '../utils/soundEngine';
import { triggerHaptic } from '../utils/haptics';

export default function RoomLobby() {
  const { code } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const { user } = useSelector((state) => state.auth);
  const { lobbyPlayers, currentRoom } = useSelector((state) => state.room);

  const [copied, setCopied] = useState(false);
  const [isReady, setIsReady] = useState(false);

  const socket = socketService.getSocket();
  const isHost = lobbyPlayers[0]?.userId === user?.id;
  const shareUrl = `${window.location.origin}/join/${code}`;

  useEffect(() => {
    // 1. Fetch Room metadata
    api.get(`/rooms/${code}`)
      .then((res) => {
        if (res.data.success) {
          dispatch(setRoom({ room: res.data.room, isHost: res.data.room.host_id === user?.id }));
          
          // Connect to socket room
          socket.emit('room:join', {
            roomId: res.data.room.id,
            roomCode: code,
            user: user || { id: -999, username: 'Guest' }
          });
        }
      })
      .catch((err) => {
        console.error('Error fetching room:', err);
      });

    // 2. Listen to lobby updates
    socket.on('room:update', (data) => {
      if (data.lobby) {
        dispatch(updateLobby({ players: data.lobby.players, hostId: data.lobby.hostId }));
      }
    });

    // 3. Listen to game start
    socket.on('game:start', (data) => {
      sound.playTurn();
      triggerHaptic('medium');
      dispatch(setGameState(data.game));
      navigate(`/game/${code}`);
    });

    return () => {
      socket.off('room:update');
      socket.off('game:start');
    };
  }, [code, user, dispatch, navigate, socket]);

  // Handle Share Room Link
  const handleShare = async () => {
    sound.playClick();
    const shareMessage = `🎲 Join my Budo game!\n\nRoom Code: ${code}\n\nJoin directly:\n${shareUrl}`;

    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Budo Multiplayer Ludo',
          text: shareMessage,
          url: shareUrl
        });
      } catch (err) {
        // Fallback
      }
    } else {
      // Direct WhatsApp intent
      const whatsappUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(shareMessage)}`;
      window.open(whatsappUrl, '_blank');
    }
  };

  const handleCopy = () => {
    sound.playClick();
    navigator.clipboard.writeText(shareUrl);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  const handleToggleReady = () => {
    sound.playClick();
    const nextReady = !isReady;
    setIsReady(nextReady);
    if (currentRoom && user) {
      socket.emit('room:ready', {
        roomId: currentRoom.id,
        userId: user.id,
        isReady: nextReady
      });
    }
  };

  const handleAddBot = () => {
    sound.playClick();
    if (currentRoom) {
      socket.emit('room:addBot', {
        roomId: currentRoom.id,
        difficulty: 'medium'
      });
    }
  };

  const handleStartGame = () => {
    sound.playVictory();
    triggerHaptic('heavy');
    if (currentRoom) {
      socket.emit('room:start', { roomId: currentRoom.id });
    }
  };

  return (
    <div className="min-h-screen bg-budo-bg pb-16 flex flex-col items-center">
      <Navbar />

      <main className="w-full max-w-md md:max-w-lg px-4 py-4 space-y-4">
        {/* Room Header Card */}
        <div className="bg-slate-900 border-2 border-slate-800 rounded-3xl p-5 shadow-2xl text-center relative overflow-hidden">
          <div className="text-xs uppercase font-bold tracking-widest text-slate-400">
            Budo Game Lobby
          </div>

          <div className="my-2 flex items-center justify-center gap-2">
            <span className="font-mono text-3xl font-black text-amber-400 tracking-wider">
              {code}
            </span>
            <button
              onClick={handleCopy}
              className="p-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 active:scale-90 transition-transform"
              title="Copy Room Link"
            >
              {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
            </button>
          </div>

          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-500/20 rounded-full border border-blue-500/40 text-blue-300 text-xs font-bold">
            <Users className="w-3.5 h-3.5" />
            <span>{lobbyPlayers.length} / {currentRoom?.max_players || 4} Players</span>
          </div>
        </div>

        {/* Player Roster */}
        <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-4 shadow-xl space-y-2.5">
          <div className="flex items-center justify-between px-1">
            <span className="text-xs font-bold uppercase text-slate-300">Players in Room</span>
            {isHost && lobbyPlayers.length < (currentRoom?.max_players || 4) && (
              <button
                onClick={handleAddBot}
                className="flex items-center gap-1 text-xs font-bold text-cyan-400 bg-cyan-500/15 hover:bg-cyan-500/25 px-2.5 py-1 rounded-xl active:scale-95 transition-all"
              >
                <Bot className="w-3.5 h-3.5" />
                <span>+ Add Bot</span>
              </button>
            )}
          </div>

          <div className="space-y-2">
            {lobbyPlayers.map((player, idx) => {
              const isPlayerHost = idx === 0;
              return (
                <div
                  key={player.userId || idx}
                  className="flex items-center justify-between p-3 bg-slate-950 border border-slate-800/80 rounded-2xl"
                >
                  <div className="flex items-center gap-3">
                    <img
                      src={player.avatarUrl || '/avatars/default.png'}
                      alt={player.username}
                      className="w-10 h-10 rounded-xl border border-slate-700 object-cover bg-slate-900"
                    />
                    <div>
                      <div className="text-xs md:text-sm font-bold text-white flex items-center gap-1.5">
                        <span>{player.username}</span>
                        {isPlayerHost && <Crown className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />}
                        {player.isBot && <Bot className="w-3.5 h-3.5 text-cyan-400" />}
                      </div>
                      <div className="text-[10px] text-slate-400">
                        {player.rankingPoints || 1000} Pts
                      </div>
                    </div>
                  </div>

                  <div>
                    {isPlayerHost ? (
                      <span className="text-[11px] font-black text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-lg border border-amber-500/30">
                        HOST
                      </span>
                    ) : player.isReady ? (
                      <span className="text-[11px] font-bold text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/30 flex items-center gap-1">
                        <CheckCircle2 className="w-3 h-3" />
                        <span>Ready</span>
                      </span>
                    ) : (
                      <span className="text-[11px] font-bold text-slate-400 bg-slate-800 px-2.5 py-1 rounded-lg flex items-center gap-1">
                        <Clock className="w-3 h-3" />
                        <span>Waiting</span>
                      </span>
                    )}
                  </div>
                </div>
              );
            })}
          </div>
        </div>

        {/* Action Controls */}
        <div className="space-y-2.5 pt-1">
          {/* Share Room Button (WhatsApp & Web Share) */}
          <button
            onClick={handleShare}
            className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs uppercase tracking-wider rounded-2xl shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 active:scale-95 transition-all"
          >
            <Share2 className="w-4 h-4" />
            <span>Share Invite via WhatsApp / App</span>
          </button>

          {/* Host: Start Game | Player: Ready Toggle */}
          {isHost ? (
            <button
              onClick={handleStartGame}
              disabled={lobbyPlayers.length < 2}
              className="w-full py-4 bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 disabled:opacity-40 text-slate-950 font-black text-xs uppercase tracking-wider rounded-2xl shadow-xl shadow-amber-500/30 flex items-center justify-center gap-2 active:scale-95 transition-all"
            >
              <Play className="w-4 h-4 fill-slate-950" />
              <span>{lobbyPlayers.length < 2 ? 'Waiting for Players (Min 2)...' : 'Start Match Now'}</span>
            </button>
          ) : (
            <button
              onClick={handleToggleReady}
              className={`w-full py-4 font-black text-xs uppercase tracking-wider rounded-2xl shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all ${
                isReady
                  ? 'bg-emerald-600 text-white shadow-emerald-600/30'
                  : 'bg-blue-600 text-white shadow-blue-600/30'
              }`}
            >
              <CheckCircle2 className="w-4 h-4" />
              <span>{isReady ? 'Ready! (Tap to Unready)' : 'Tap When Ready'}</span>
            </button>
          )}
        </div>
      </main>
    </div>
  );
}
